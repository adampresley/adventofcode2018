...
func main() {
	...
	fmt.Printf("Starting frequency: %d\n\n", startingFrequency)

	for !duplicateFrequencyFound {
		for scanner.Scan() {
			if scanner.Text() != "" {
				scannedInput := scanner.Text()
				frequencyAdjustment, _ := strconv.Atoi(scannedInput)

				frequency += frequencyAdjustment
				fmt.Printf("Adjustment: %d, new frequency: %d\n", frequencyAdjustment, frequency)

				if _, ok = seenFrequencies[frequency]; !ok {
					seenFrequencies[frequency] = 1
				} else {
					seenFrequencies[frequency]++
					if seenFrequencies[frequency] > 1 {
